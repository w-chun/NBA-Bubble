...
(function () {
  ...
  var svg = d3.select("#bubble").append("svg").attr("class", "svg").attr("preserveAspectRatio", "xMinYMin meet").attr("viewBox", "0 0 " + width + " " + height).classed("svg-content", true).append("g").attr("transform", "translate(0,0)");
  ...
  var queue = d3.queue().defer(d3.csv, "https://w-chun.github.io/NBA-Bubble/data/NBAFG_2016_2017.csv").await(ready);
  ...
  function ready(error, datapoints) {
    ...
    season.selectAll("option").data(years).enter().append("option").text(function (d) {
      return d.key + " : " + d.value;
    });

    var team = d3.select("#team");

    var teams = d3.nest().key(function (d) {
      return d.team;
    }).rollup(function (arr) {
      return arr.length;
    }).entries(datapoints);
    ...
    team.selectAll("option").data(teams).enter().append("option").text(function (d) {
      return d.key;
    ...
    team.on('change', function () {
      d3.selectAll(".player").attr('visibility', 'visible');
      var value = team.property('value');
      if (value != "All Teams") {
        d3.selectAll('.player').filter(function (d) {
          return d.team != value;
    ...
    var playerInfo = d3.select("#bubble").append("div").style("visibility", "hidden").style("position", "absolute").style("background-color", "rgba(29, 66, 138, 0.7)").style("padding", "10px 20px").style("border-radius", "5px").style("font-family", "sans-serif").style("font-size", "1vw").style("color", "white");
    ...
    var circles = svg.selectAll(".player").data(datapoints).enter().append("circle").attr("class", "player").attr("id", function (d) {
      ...
      }).strength(.12)).force("y", d3.forceY(function (e) {
        if (e.idx == i) {
          return height / 4;
        } else {
          return height / 2;
        }
      }).strength(.12)).alphaTarget(.16).restart();
    ...
    d3.select("#percent").on('click', function () {
      simulation.force("x", forceXSeparate).force("y", forceY).alphaTarget(.16).restart();
      playerInfo.style("visibility", "hidden");
      percentages.html("<div>Under 35%</div>\n            <div>35% - 50%</div>\n            <div>Greater than 50%</div>").style("visibility", "visible").style("top", "220px");
    });

    d3.select("#reset").on('click', function () {
      simulation.force("x", forceXReset).force("y", forceY).alphaTarget(.16).restart();
      playerInfo.style("visibility", "hidden");
      percentages.style("visibility", "hidden");
    });
