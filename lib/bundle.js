...
(function () {
  ...
  var forceXReset = d3.forceX(window.screen.width / 2).strength(.1);
  ...
  function ready(error, datapoints) {
    ...
    var playerInfo = d3.select("#bubble").append("div").style("visibility", "hidden").style("position", "absolute").style("background-color", "rgba(29, 66, 138, 0.7)").style("padding", "10px").style("border-radius", "5px").style("font", "14px sans-serif").style("color", "white");
    ...
    var circles = svg.selectAll(".player").data(datapoints).enter().append("circle").attr("class", "player").attr("id", function (d) {
      ...
      }).strength(.1)).force("y", d3.forceY(function (e) {
        if (e.idx == i) {
          return height / 4;
        } else {
          return height / 2;
        }
      }).strength(.1)).alphaTarget(.28).restart();
    ...
    d3.select("#percent").on('click', function () {
      simulation.force("x", forceXSeparate).force("y", forceY).alphaTarget(.28).restart();
    ...
    d3.select("#reset").on('click', function () {
      simulation.force("x", forceXReset).force("y", forceY).alphaTarget(.28).restart();
      playerInfo.style("visibility", "hidden");
      percentages.style("visibility", "hidden");
    });
