...
(function () {
  ...
  var radiusScale = d3.scaleSqrt().domain([6, 2729]).range([20, 90]);
  ...
  function ready(error, datapoints) {
    ...
    d3.select("#percent").on('click', function () {
      simulation.force("x", forceXSeparate).force("y", forceY).alphaTarget(.10).restart();
    ...
    d3.select("#reset").on('click', function () {
      simulation.force("x", forceXReset).force("y", forceY).alphaTarget(.10).restart();
      playerInfo.style("visibility", "hidden");
      percentages.style("visibility", "hidden");
    });
